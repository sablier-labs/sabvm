...
pub fn validate_env<SPEC: Spec, DB: Database>(env: &Env) -> Result<(), EVMError<DB::Error>> {
    ...
    if tx.is_system_transaction.unwrap_or(false) && SPEC::enabled(SpecId::REGOLITH) {
        return Err(InvalidTransactionReason::DepositSystemTxPostRegolith.into());
    }
...
mod tests {
    ...
    fn test_commit_mint_value() {
        ...
        db.insert_account_info(
            caller,
            AccountInfo {
                balances: Balances::from([(BASE_ASSET_ID, U256::from(1000))]),
                ..Default::default()
            },
        );
    ...
    fn test_remove_l1_cost() {
        ...
        db.insert_account_info(
            caller,
            AccountInfo {
                balances: Balances::from([(BASE_ASSET_ID, U256::from(1049))]),
                ..Default::default()
            },
        );
        let mut context: Context<(), InMemoryDB> = Context::new_with_db(db);
    ...
    fn test_remove_l1_cost_lack_of_funds() {
        ...
        db.insert_account_info(
            caller,
            AccountInfo {
                balances: Balances::from([(BASE_ASSET_ID, U256::from(48))]),
                ..Default::default()
            },
        );
        let mut context: Context<(), InMemoryDB> = Context::new_with_db(db);
